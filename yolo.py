...
#                    _,crop_image = cv2.threshold(crop_image, 127, 255, cv2.THRESH_BINARY)
                    ...
                    _, binary_img = cv2.threshold(crop_image, 200, 255, cv2.THRESH_BINARY)  # 使用INV因为细胞是亮的
                    
                    # 查找轮廓  
                    contours, _ = cv2.findContours(binary_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  
                    
                    # 创建一个PIL图像来绘制结果（注意：我们需要将OpenCV图像从BGR转换为RGB）  
...
###############       模板匹配获得的坐标得到相对坐标         ##############################################################
            ...
            print("坐标已成功写入coordinates.csv文件")
...
############################################################################################################## 
            ...
            number_size = draw.textsize(label_number, font)
                #class_detect = class_detect.encode('utf-8')
            label = label.encode('utf-8')
                # print(label, top, left, bottom, right, (center_x, center_y))
            ...
            class_data = '细胞类别：{}'.format(class_detect)
            list_class.append(class_data)
                #list_x.append(x_data)
                #list_y.append(y_data)
############################################################################################################
#####################   细胞中心十字   ######################################################################    
            # 明亮的黄绿色 (RGB格式)
            BRIGHT_GREEN = (0, 255, 0)# 鲜艳的苹果绿
            for i in range(thickness):
 ...
 #          for i in range(thickness):
 #                  draw.rectangle([center_x + 2, center_y + 8, center_x - 2, center_y - 8], fill=self.colors[c], outline=self.colors[c])
 #                  draw.rectangle([center_x + 8, center_y + 2, center_x - 8, center_y - 2],
...
#            draw.rectangle([(tx + 245), (ty + 245), (tx - 245), (ty - 245)], outline=BLUE)
#            draw.rectangle([(tx + 246), (ty + 246), (tx - 246), (ty - 246)], outline=BLUE)
